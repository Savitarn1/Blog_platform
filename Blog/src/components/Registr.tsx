...
const Registr = () => {
  ...
  return (
    <div>
      <h1 className='text-4xl font-bold text-center my-5'>Registr Page</h1>
      <form onSubmit={handleSubmit(submithandler)} className='flex flex-col gap-5 w-[600px] mx-auto'>
        <input
          className='mx-auto outline-none w-full border-2 border-slate-400 px-2 text-lg py-1 rounded-md'
          type='text'
          {...register('login' , {required : true})}
        />
        {errors.login && <span className='text-red-600'>{errors.login.message}</span>}
        <label
          className='mx-auto w-full border-2 border-slate-400 px-2 text-lg py-1 rounded-md flex items-center'
          htmlFor='password'
        >
          <input
            id='password'
